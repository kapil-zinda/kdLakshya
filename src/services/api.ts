...
export class ApiService {
  ...
  static async getOrganization(
    ...
    const cached = apiCache.get<OrganizationResponse>(cacheKey, 300000);
  ...
  static async getSiteConfig(orgId: string): Promise<SiteConfigResponse> {
    ...
    const cached = apiCache.get<SiteConfigResponse>(cacheKey, 300000);
    if (cached) return cached;
  ...
  static async getAbout(orgId: string): Promise<AboutResponse> {
    ...
    const cached = apiCache.get<AboutResponse>(cacheKey, 300000);
    if (cached) return cached;
  ...
  static async getHero(orgId: string): Promise<HeroResponse> {
    ...
    const cached = apiCache.get<HeroResponse>(cacheKey, 300000);
    if (cached) return cached;
  ...
  static async getBranding(orgId: string): Promise<BrandingResponse> {
    ...
    const cached = apiCache.get<BrandingResponse>(cacheKey, 300000);
    if (cached) return cached;
  ...
  static async getPrograms(orgId: string): Promise<ProgramsResponse> {
    ...
    const cached = apiCache.get<ProgramsResponse>(cacheKey, 300000);
    if (cached) return cached;
  ...
  static async getStats(orgId: string): Promise<StatsResponse> {
    ...
    const cached = apiCache.get<StatsResponse>(cacheKey, 300000);
    if (cached) return cached;
  ...
  static async getNews(orgId: string): Promise<NewsListResponse> {
    ...
    const cached = apiCache.get<NewsListResponse>(cacheKey, 300000);
    if (cached) return cached;
  ...
  static async fetchAllData(
    ...
    const cached = apiCache.get<FetchAllDataResult>(cacheKey, 300000);
    if (cached) return cached;

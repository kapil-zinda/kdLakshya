...
import React, {
  ...
  export const TaskProvider: React.FC<TaskProviderProps> = ({ children }) => {
    ...
    const addTask = (newTask: {
      ...
      if (Array.isArray(tasks)) {
        const updatedTasks = [
          ...tasks,
          createTask(newTask.title, newTask.status, newTask.priority, newTask.need, newTask.label),
        ];
  ...
  const createTask = (title: string, status: string, priority: string, need: string, label: string,): Task => {
    return {
      id: uuidv4(),
      title,
      status,

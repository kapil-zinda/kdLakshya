  ...
  const columns: ColumnDef<User>[] = [
    {
      ...
      ), // Access name directly from row.original
    },

    {
      ...
      cell: ({ row }) => (
        <div className="lowercase">{row.original.attributes.email}</div>
...

    {
      ...
      cell: ({ row }) => (
        <div className="lowercase">{row.original.attributes.created_ts}</div>
      ...
      cell: ({ row }) => {
        ...
        const handleDeleteUser = async (user: User) => {
          const res = await axios.delete(
            `${BaseURLAuth}/users/${user.id}?permanent=true`,
            {
              headers: {
